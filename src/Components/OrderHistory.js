...
export default function OrderHistory() {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            const response = await fetch(`https://theflavr.in/orders/getHistory/completed?outletid=${localStorage.getItem('selectedOutlet')}&date=${filter.date+1}&month=${filter.month}&year=${filter.year}`, {
                method: "GET",
                headers: {
    ...
    useEffect(() => {
        const fetchData = async () => {
            setLoading(true)
            const response = await fetch(`https://theflavr.in/orders/getHistory/completed?outletid=${localStorage.getItem('selectedOutlet')}&date=${Number(filter.date)+1}&month=${Number(filter.month)}&year=${Number(filter.year)}`, {
                method: "GET",
                headers: {
                    "Content-Type": "application/json",
                    "Authorization": "Bearer " + localStorage.getItem('token')
